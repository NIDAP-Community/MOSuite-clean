setup_cli_workspace <- function(prefix = "mosuite_clean_test_") {
...
expect_outputs_created <- function(results_dir) {
  moo_path <- file.path(results_dir, "moo", "moo.rds")

  expect_true(file.exists(moo_path), info = "Output moo.rds should be created")
  expect_true(
    file.info(moo_path)$size > 0,
    info = "Output moo.rds should be non-empty"
  )
